    ...
    options: async (value: string, { req }) => {
      ...
      if (!ObjectId.isValid(value)) {
        throw new ErrorWithStatus({
          message: USERS_MESSAGES.INVALID_FOLLOWED_USER_ID, //trong message.ts thêm INVALID_user_id: 'Invalid user id'followed user id'
...
//khi đăng nhập thì ta sẽ có 1 req.body gồm email và password
      ...
      Authorization: {
        ...
        custom: {
          ...
          options: async (value: string, { req }) => {
            ...
            //nếu xuống đc đây thì tức là accessToken có rồi
            //cần verify nó có hợp lệ hay không và lấy payload ra lưu vào req
            try {
  ...
  checkSchema(
    {
      email_verify_token: {
        ...
        custom: {
          options: async (value, { req }) => {
            //nếu email_verify_token k gửi lên thì ta sẽ throw error
            if (!value) {
              throw new ErrorWithStatus({
